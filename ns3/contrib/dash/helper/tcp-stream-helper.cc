...
ApplicationContainer
TcpStreamClientHelper::Install (std::vector <std::pair <Ptr<Node>, std::string> > clients, uint16_t id_offset) const
{
  ApplicationContainer apps;
  for (uint i = 0; i < clients.size (); i++)
    {
      apps.Add (InstallPriv (clients.at (i).first, clients.at (i).second, i + id_offset));
    }
